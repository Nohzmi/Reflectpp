...
namespace reflectpp
{
	namespace details
	{
		...
		struct associative_container<std::unordered_map<Key, T>> final
		{
			REFLECTPP_INLINE static auto get_data() REFLECTPP_NOEXCEPT;
		};

		template<typename Key, typename T>
		struct is_associative_container<std::unordered_map<Key, T>> final : std::true_type {};

		template<typename Key, typename T>
		struct has_value_type<std::unordered_map<Key, T>> final : std::true_type {};
	}
}
