...
namespace reflectpp
{
	...
	{
		if (m_type != nullptr && m_type->m_enumeration == nullptr)
	...
	{
		return name != nullptr ? registration(details::registry::get_instance().add_enumeration<T>(name)) : registration();
	}
	...
	{
		if (m_type != nullptr && m_type->m_enumeration == nullptr && name != nullptr)
